...
| 自动重试 | CADC 查询、PS1 cutout 和原始 warp FITS 下载遇到超时、断线、429 或常见 5xx 临时错误时自动重试。 |
| 断点友好下载 | 原始 FITS 使用 `.part` 临时文件，完整下载并校验后再改名，避免残缺文件被误认为下载成功。 |
...
如果未设置上述变量，脚本会尝试通过图形对话框请求输入。若用户取消输入、图形界面不可用或 CasJobs 连接失败，脚本会跳过目录匹配并继续执行其余流程。
...
默认最多尝试 5 次。失败后的等待时间采用指数退避，默认依次约为 `2`、`4`、`8`、`16` 秒。连接失败、读取超时、HTTP 408、429、500、502、503、504 会自动重试；404 等通常属于永久错误的普通 4xx 不会反复重试。

PS1 cutout 单张图像即使最终下载失败，也只会将该图像标记为失败并继续下一张，不再让整个脚本退出。原始 warp FITS 同样会在单个文件达到最大尝试次数后记录失败并继续处理其余文件。

原始 FITS 会先保存为 `<文件名>.part`，下载完成且文件非空、并在服务器提供 `Content-Length` 时通过大小校验后，再原子改名为正式 `.fits` 文件。这样中途断线留下的残缺文件不会在下一次运行时被误判为已经下载完成。
...
此前原始 warp FITS 路径由 `tab2path()` 生成时已经以 `.fits` 结尾，而下载函数又重复追加了一次扩展名，可能生成 `*.fits.fits` URL。当前版本已修正该问题。

同时修正了缩略图下载失败后的表格筛选逻辑：失败图像对应的观测行不会再错误进入 `goodtab`，从而避免图像列表与观测表错位。

## 注意事项

脚本会访问多个第三方数据服务，运行时间、结果数量和下载体积取决于目标、查询时间范围与服务状态。部分查询或下载失败时，脚本会输出警告并尽可能继续执行；使用结果前应检查控制台日志与各输出表的完整性。

已存在且非空的原始 FITS 文件会被跳过。如果怀疑旧版本曾留下错误或损坏的正式 `.fits` 文件，建议先人工核验或删除该文件后重新运行。
